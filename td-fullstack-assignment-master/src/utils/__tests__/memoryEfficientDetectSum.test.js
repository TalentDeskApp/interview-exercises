...
import { detectSumsMemoryEfficient } from '../utils';

describe('Detect memory efficient sums', () => {
  it('should fail if input is not an array', () => {
    expect(() => detectSumsMemoryEfficient()).to.throw('Input is not an array');
  });
  ...
  it.each([
    ...
    ({ input, expectedLength }) => {
      const result = detectSumsMemoryEfficient(input);
